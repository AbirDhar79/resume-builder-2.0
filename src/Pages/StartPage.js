...
import "./Styles/StartPage.css"; // Ensure this matches the exact file name
import { Box, Button, Stack } from "@mui/material";
import { useNavigate } from "react-router-dom";
import { connect } from "react-redux";
import { Navbar, BlackScreen } from ".";
...
const TemplateWrapper = ({ children }) => {
  return <div className="template-box">{children}</div>;
};
...
const StartPage = (props) => {
  ...
  const goToFillDetails = (id) => {
    props.updateTemplateId(id);
              ...
              <TemplateWrapper key={tmpl.id}>
                ...
                >
                  <img
                    style={{ width: "75%" }}
                    className="template-img"
                    src={tmpl.template_img}
                    alt={tmpl.template_name}
                  />
                  <BlackScreen />
                  <Button
                    className="use-template-btn"
                    onClick={() => goToFillDetails(tmpl.id)}
                    size="medium"
                    variant="contained"
                    color="secondary"
                  >
